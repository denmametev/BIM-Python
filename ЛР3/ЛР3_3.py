    ...
    for char in text:
        if char.isalpha():  # Проверка, является ли символ буквой
            ...
            if char in letter_count:
                letter_count[char] += 1  # Увеличиваем количество этой буквы
            else:
                letter_count[char] = 1  # Добавляем букву в словарь, если её нет

    return letter_count


def calculate_frequency(letter_count):
    all_letters = sum(letter_count.values())  # Общее количество букв
    frequency = {}
    for letter, count in letter_count.items():
        frequency[letter] = round(count / all_letters, 2)  # Частота буквы
    return frequency
